...
<?php
    ...
    echo "<table border=4>";
    echo "<tr><th></th><th>#</th><th>GUIID</th><th>Requires Activation</th></tr>";
    $num_rows = 0;
    while($xml_r->read()) {
        if($xml_r->getAttribute("id")) {
     ...
     //     printf("<tr><td><input type=checkbox onclick=\"m_deleteMe()\" id=\"wb_%d\" name=\"wb_%d\"></td><td>%d</td><td>%s</td><td>%s</td></tr>", $num_rows, $num_rows, $num_rows, $xml_r->getAttribute("id"), $xml_r->readInnerXML());
            ...
            printf("<tr><td><img src=\"/home/assets/buttons/delete_row_button.png\" class=\"actionButton\" onclick=\"m_deleteMe(wb_%s, '%s', '%s')\" id=\"wb_%d\" name=\"wb_%d\"></td><td>%d</td><td>%s</td><td>%s</td></tr>", ++$num_rows, $xml_r->getAttribute("id"), $api_node, $num_rows, $num_rows, $num_rows, $xml_r->getAttribute("id"), $xml_r->readInnerXML());
        }
